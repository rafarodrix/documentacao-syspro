...
export function getCorrelationIdFromRequest(request: RequestLoggerLike): string {
  return request.headers.get("x-correlation-id") ?? request.headers.get("x-request-id") ?? crypto.randomUUID();
}

export function createRequestLogger(request: RequestLoggerLike, options: RequestLoggerOptions = {}) {
