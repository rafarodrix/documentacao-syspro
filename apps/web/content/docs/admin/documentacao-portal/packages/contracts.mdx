...
// Atualização — schema próprio: campos obrigatórios mantidos, clearáveis com nullable()
export const updateContractSchema = z.object({
  id: z.string().min(1),
  // Campos obrigatórios (mesmos que create — update envia todos)
  companyId: z.string().min(1),
  percentage: z.coerce.number().min(0.0001).max(100),
  minimumWage: z.coerce.number().min(1),
  taxRate: z.coerce.number().min(0).max(100),
  ...
  allowTaxOverride: z.boolean().optional().default(false),
  // Campos clearáveis — aceitam null para SET NULL no banco
  endDate: z.string().nullable().optional(),
  contractNumber: z.string().max(80).nullable().optional(),
  notes: z.string().max(1000).nullable().optional(),
})
// mesmas refinements do create (taxRate+programmerRate <= 100 e endDate > startDate)
...
**Campos obrigatórios no update:** os campos numéricos e `companyId` permanecem requeridos — o update de contrato sempre envia o formulário completo. Somente `endDate`, `contractNumber` e `notes` aceitam `null` para que o Prisma execute `SET NULL` no banco.
