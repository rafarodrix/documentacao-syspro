---
title: Módulo de Tickets — Arquitetura Completa
description: Arquitetura completa do módulo de tickets — estados, workflow, SLA, RBAC, notificações WhatsApp e integração Chatwoot.
...
> Detalhes complementares na seção `Apps > API > Automação`.
