...
type SecretTokenOptions = {
  headerName: string;
  queryName?: string;
  allowBearer?: boolean;
};

export function extractSecretToken(request: Request, options: SecretTokenOptions): string {
...
export function isValidSecretToken(request: Request, expectedSecret: string, options: SecretTokenOptions): boolean {
...
export function isValidHmacSignature(payload: string, providedSignature: string | null, secret: string): boolean {
