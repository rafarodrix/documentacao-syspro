...
export function createRemoteAddressBookPort(): RemoteAddressBookPort {
  return {
    async listAddressBook(input: ListAddressBookInput): Promise<ListAddressBookOutput> {
    ...
    async listAddressBookCredentials(_input: ListAddressBookCredentialsInput): Promise<ListAddressBookCredentialsOutput> {
    ...
    async createAddressBookCredential(input: CreateAddressBookCredentialInput): Promise<CreateAddressBookCredentialOutput> {
      ...
      if (scope === "COMPANY" && companyId) {
        ...
        if (!company) {
          throw new Error("ADDRESS_BOOK_COMPANY_NOT_FOUND");
        }
      }
      ...
      if (!integrationKey) {
        throw new Error("ADDRESS_BOOK_INTEGRATION_KEY_INVALID");
      }
    ...
    async rotateAddressBookCredential(input: RotateAddressBookCredentialInput): Promise<RotateAddressBookCredentialOutput> {
      const current = await prisma.remoteAddressBookCredential.findUnique({ where: { id: input.credentialId } });
      if (!current) {
        throw new Error("ADDRESS_BOOK_CREDENTIAL_NOT_FOUND");
      }
    ...
    async revokeAddressBookCredential(input: RevokeAddressBookCredentialInput): Promise<RevokeAddressBookCredentialOutput> {
      ...
      if (credential.status === "REVOKED") {
        return {
          revoked: true,
          alreadyRevoked: true,
          message: "Credencial ja estava revogada.",
        };
      }
