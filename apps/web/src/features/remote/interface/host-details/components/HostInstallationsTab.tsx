    ...
    <div className="space-y-4">
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-4">
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.9fr)_auto] lg:items-end">
                ...
                {canManageInstallations ? (
                  ...
                  >
                    ...
                    {isCreatingManualInstallation ? "Adicionando..." : "Adicionar instalação"}
                  </Button>
                ) : null}
              </div>
          ...
          <div className="rounded-2xl border border-border/50 bg-muted/15 p-5">
            <div className="space-y-4">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Instalações vinculadas</p>
                ...
                return (
                  <div key={entry.id} className="rounded-xl border border-border/50 bg-muted/15 p-4 text-sm text-muted-foreground">
                    ...
                    <details className="mt-3 rounded-lg border border-border/40 bg-background/40 p-3">
                      <summary className="cursor-pointer text-sm font-medium text-foreground">Informações do servidor</summary>
                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        <div className="rounded-lg border border-border/40 bg-background/40 p-3">
                          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Servidor</p>
                          <p className="mt-1 text-sm text-foreground">
                            {entry.companyId ? companyContext?.serverHost ?? "Nao configurado" : "Sem vínculo"}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border/40 bg-background/40 p-3">
                          ...
                          <p className="mt-1 text-sm text-foreground">
                            {entry.companyId
                              ? companyContext?.serverPort
                                ? String(companyContext.serverPort)
                                : "Nao configurado"
                              : "Sem vínculo"}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border/40 bg-background/40 p-3">
                          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Conexão</p>
                          <p className="mt-1 text-sm text-foreground">
                            {entry.companyId ? companyContext?.serverProtocol ?? "Nao configurado" : "Sem vínculo"}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border/40 bg-background/40 p-3">
                          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Url Path (ISAPI)</p>
                          <p className="mt-1 break-all text-sm text-foreground">
                            {entry.companyId ? companyContext?.iisIsapiPath ?? "Nao configurado" : "Sem vínculo"}
                          </p>
                        </div>
                      </div>
                    </details>
                    ...
                    <details className="mt-3 rounded-lg border border-border/40 bg-background/40 p-3">
                      <summary className="cursor-pointer text-sm font-medium text-foreground">Observações</summary>
                      <div className="mt-3 rounded-lg border border-border/40 bg-background/40 p-3">
                        <p className="whitespace-pre-wrap text-sm text-foreground">
                          {entry.companyId
                            ? companyContext?.observacoes ?? "Sem observações operacionais para esta empresa."
                            : "Sem vínculo com empresa do cadastro para exibir observações."}
                        </p>
                      </div>
                    </details>
