...
import { ArrowLeft, AlertCircle, Clock3, Hash, Timer, Flag, UserRound, Loader2, Sparkles, MessageSquareShare, Phone, User, ExternalLink } from "lucide-react";
...
export function TicketDetails({ ticket, articles, isAdmin, error }: TicketDetailsProps) {
    ...
    const [releaseType, setReleaseType] = useState<"BUG" | "MELHORIA" | "">(ticket?.releaseType === "BUG" || ticket?.releaseType === "MELHORIA" ? ticket.releaseType : "");
    ...
    const runQuickAction = (action: "assume" | "priority_high" | "macro_followup") => {
        ...
        startTransition(async () => {
            try {
                ...
                const label = action === "assume" ? "Ticket assumido com sucesso." : action === "priority_high" ? "Prioridade alterada para alta." : "Macro aplicada com sucesso.";
                toast.success(label);
                router.refresh();
            } catch {
