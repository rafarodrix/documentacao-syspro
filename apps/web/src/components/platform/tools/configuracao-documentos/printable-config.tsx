...
export const PrintableConfig = React.forwardRef<HTMLDivElement, PrintableConfigProps>(({ data }, ref) => {
    ...
    // Helper para labels de CrÃ©dito/DÃ©bito
    const getFinalidadeLabel = (cod: string) => {
        if (cod === "5") {
            const motivo = TIPOS_NOTA_CREDITO.find(t => t.value === data.tpNFCredito)?.label;
            return `Nota de CrÃ©dito - ${motivo || "Motivo nÃ£o informado"}`;
        }
        if (cod === "6") {
            const motivo = TIPOS_NOTA_DEBITO.find(t => t.value === data.tpNFDebito)?.label;
            return `Nota de DÃ©bito - ${motivo || "Motivo nÃ£o informado"}`;
        }
        const map = { "1": "Normal", "2": "Complementar", "3": "Ajuste", "4": "DevoluÃ§Ã£o" };
    ...
    return (
        <div ref={ref} className="p-8 font-sans text-black bg-white print-content">
            ...
            <div className="border-b-2 border-black pb-4 mb-6 flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-bold uppercase tracking-tight">Ficha de ParametrizaÃ§Ã£o</h1>
                    <p className="text-sm text-gray-600">ConfiguraÃ§Ã£o de Modelo Fiscal ERP</p>
                </div>
                <div className="text-right text-xs">
                    <p><strong>EmissÃ£o:</strong> {new Date().toLocaleDateString()}</p>
            ...
            {/* 1. Dados Gerais */}
            <div className="mb-6">
                <h3 className="bg-gray-100 p-2 font-bold uppercase text-xs border border-gray-300 mb-2">1. IdentificaÃ§Ã£o do Modelo</h3>
                <table className="w-full text-sm border border-gray-300">
                    <tbody>
                        <tr className="border-b border-gray-200">
                            <td className="p-2 font-bold bg-gray-50 w-1/4">DescriÃ§Ã£o Interna:</td>
                            <td className="p-2 font-medium" colSpan={3}>{data.descricao}</td>
                        </tr>
                        <tr className="border-b border-gray-200">
                            <td className="p-2 font-bold bg-gray-50">Grupo de NegÃ³cio:</td>
                        ...
                        <tr className="border-b border-gray-200">
                            ...
                            <td className="p-2 font-bold bg-gray-50 w-1/4">SÃ©rie:</td>
                ...
                <table className="w-full text-sm border border-gray-300 text-center">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-300">
                            <th className="p-2 text-left">CenÃ¡rio Fiscal</th>
                    ...
                    <tbody>
                        <tr className="border-b border-gray-200">
                            <td className="p-2 text-left font-bold">1. TributaÃ§Ã£o Normal</td>
                        ...
                        <tr className="border-b border-gray-200">
                            <td className="p-2 text-left font-bold">2. SubstituiÃ§Ã£o TributÃ¡ria (ST)</td>
                        ...
                        <tr>
                            <td className="p-2 text-left font-bold">4. Exterior (ExportaÃ§Ã£o)</td>
            ...
            <div className="mb-8">
                <h3 className="bg-gray-100 p-2 font-bold uppercase text-xs border border-gray-300 mb-2">3. Regras de AutomaÃ§Ã£o (Checklist)</h3>
                ...
                {data.comportamentos && data.comportamentos.length > 0 ? (
                        ...
                        {data.comportamentos.map(code => {
                            ...
                            return (
                                <div key={code} className="flex items-start gap-2">
                                    ...
                                    <span>{comp?.label || "Comportamento nÃ£o catalogado"}</span>
                                </div>
                            )
                        })}
                    </div>
                    ...
                    <div className="border border-gray-300 p-4 text-center text-gray-500 italic text-sm">
                        Nenhuma regra de automaÃ§Ã£o vinculada a este modelo.
                    </div>
                )}
            </div>
            ...
            <div className="mt-16 pt-8 border-t border-black grid grid-cols-2 gap-16">
                <div>
                    ...
                    <p className="text-xs uppercase font-bold text-center">ResponsÃ¡vel TÃ©cnico (TI/Consultoria)</p>
                </div>
                <div>
                    <div className="border-b border-black mb-2"></div>
                    <p className="text-xs uppercase font-bold text-center">ResponsÃ¡vel Fiscal (AprovaÃ§Ã£o)</p>
                </div>
            </div>
        </div>
    );
