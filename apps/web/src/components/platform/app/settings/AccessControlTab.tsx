...
const MODULE_LABELS: Record<string, string> = {
  ...
  contracts: "Contratos",
  crm: "CRM",
  remote: "Infraestrutura",
  agents: "Agentes da Infraestrutura",
...
export function AccessControlTab({ adminView }: AccessControlTabProps) {
  ...
  const orderedPermissionGroups = useMemo(() => {
    ...
    const ordered = MODULE_ORDER
      .filter((moduleKey) => groupedPermissions[moduleKey]?.length)
      .map((moduleKey) => {
        seen.add(moduleKey);
        return [moduleKey, groupedPermissions[moduleKey]] as const;
      });

    const remaining = Object.entries(groupedPermissions)
      .filter(([moduleKey]) => !seen.has(moduleKey))
      .sort(([left], [right]) => left.localeCompare(right));

    return [...ordered, ...remaining];
  }, [groupedPermissions]);
  const selectedPermissions = useMemo(
    () => permissions.filter((permission) => profileForm.permissions.includes(permission.key)),
    [permissions, profileForm.permissions],
  );
  const selectedPermissionsByModule = useMemo(() => {
    return selectedPermissions.reduce<Record<string, typeof selectedPermissions>>((acc, permission) => {
      if (!acc[permission.module]) acc[permission.module] = [];
      acc[permission.module].push(permission);
      return acc;
    }, {});
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Organize a matriz, os perfis e os vinculos em uma unica area administrativa.
                </CardDescription>
              </div>
            </div>
        ...
        <TabsContent value="profiles" className="space-y-6">
          <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      {isEditingProfile
                        ? "Ajuste os dados do perfil e revise a lista de permissoes ao lado."
            ...
            <div className="space-y-6">
              <Card className="border-border/50 shadow-sm bg-background/60 backdrop-blur-sm">
                <CardHeader>
                  <CardTitle>{isEditingProfile ? "Permissoes do Perfil" : "Resumo da Selecao"}</CardTitle>
                  <CardDescription>
                    {selectedPermissions.length > 0
                      ? `${selectedPermissions.length} permissoes selecionadas.`
                      : "Selecione permissoes para compor o perfil."}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {selectedPermissions.length === 0 ? (
                    <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
                      Nenhuma permissao selecionada no momento.
                    </div>
                  ) : (
                    Object.entries(selectedPermissionsByModule).map(([moduleKey, modulePermissions]) => (
                      <div key={moduleKey} className="rounded-lg border border-border/50 p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {modulePermissions.map((permission) => (
                            <Badge key={permission.key} variant="secondary" className="max-w-full">
                    ...
                    <div key={profile.id} className="rounded-lg border border-border/50 p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant={profileForm.id === profile.id ? "secondary" : "ghost"}
                            size="sm"
                            disabled={isPending}
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Visualize por permissao quais perfis recebem acesso em cada modulo.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent>
              {!enabled ? (
                ...
                <div className="space-y-5">
                  {orderedPermissionGroups.map(([moduleKey, modulePermissions]) => (
                    <div key={moduleKey} className="rounded-md border overflow-hidden">
                      <div className="border-b bg-muted/30 px-4 py-3">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="text-sm font-semibold">{getModuleLabel(moduleKey)}</p>
                            <p className="text-xs text-muted-foreground">{modulePermissions.length} permissoes mapeadas</p>
                          </div>
                          <Badge variant="outline">{modulePermissions.length}</Badge>
                        </div>
                      </div>
