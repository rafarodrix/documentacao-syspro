...
interface ReleaseCardProps {
  release: Release;
  onOpenDetails: (release: Release) => void;
}
...
export function ReleaseCard({ release, onOpenDetails }: ReleaseCardProps) {
  ...
  return (
    <button
      ...
      className="block group w-full text-left h-full focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-xl"
      aria-label={
        hasVideo
          ? `Abrir detalhes e video da release ${release.id}`
          : `Abrir detalhes da release ${release.id}`
      }
    >
      <Card
        className={cn(
      ...
      >
        <CardHeader className="p-4 pb-2">
          <div className="flex items-center justify-between">
            <Badge
              variant="outline"
              className={cn("gap-1.5 pr-3 font-medium transition-colors", typeConfig.badgeClass)}
            >
        ...
        <CardContent className="p-4 pt-2">
          ...
          <p className="text-sm text-muted-foreground line-clamp-3">{release.summary}</p>
        </CardContent>
