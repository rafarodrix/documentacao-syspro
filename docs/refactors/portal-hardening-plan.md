...
- **ID:** `QUAL-P2-002`
  - **Prioridade:** P2
  - **Evidência:** `jscpd` apontou 3.68% de duplicação total no repositório (529 clones).
  - **Arquivos:** Módulos em `apps/web/src/features/*` e `apps/api/src/modules/*`.
  - **Risco:** Manutenibilidade reduzida e divergência de regras de negócio entre interfaces.
  - **Causa Raiz:** Duplicação de componentes de tabela/filtro e formatadores de apresentação entre features.
  - **Correção:** Refatorar duplicidades de alta relevância (contratos e utilitários) para `packages/shared` ou `packages/ui`.
  - **Teste:** `npx jscpd apps packages`.
  - **Complexidade:** Média
  - **Critério de Aceite:** Redução da duplicação para menos de 3.0%.
