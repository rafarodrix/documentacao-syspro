...
export default async function AdminTicketsPage() {
    ...
    // Contadores Rápidos
    const totalOpen = ticketList.length;
    const highPriority = ticketList.filter((t: any) => t.priority === 3).length;
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <Card className="bg-blue-500/5 border-blue-500/20">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-blue-600 dark:text-blue-400">Fila de Atendimento</CardTitle>
                                    ...
                                    <TableRow key={ticket.id} className="group hover:bg-muted/40 transition-colors">
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col">
                                                ...
                                                {/* Futuramente: Mostrar nome do cliente aqui se expandirmos a query */}
...
function StatusBadge({ status }: { status: string }) {
    ...
    if (['novo', 'new'].includes(s)) style = 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800';
    if (['aberto', 'open'].includes(s)) style = 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800';
    if (['pendente', 'pending'].includes(s)) style = 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800';

    return (
        <Badge variant="outline" className={`border ${style} font-medium capitalize px-2.5 py-0.5`}>
            {status}
        </Badge>
    );
}
...
function PriorityBadge({ priority }: { priority: number }) {
    if (priority === 3) { // High
        return <Badge variant="destructive" className="text-[10px] font-bold px-2">Alta</Badge>;
    }
    if (priority === 1) { // Low
        return <Badge variant="secondary" className="text-[10px] text-muted-foreground px-2">Baixa</Badge>;
    }
    return <Badge variant="outline" className="text-[10px] text-muted-foreground px-2">Normal</Badge>;
}